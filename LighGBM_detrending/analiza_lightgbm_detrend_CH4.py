...
# ---------------------------------------------------------
# 3. KROK 1: usunięcie trendu liniowego (dopasowanego TYLKO na treningu!)
# ---------------------------------------------------------
model_trend = LinearRegression()
model_trend.fit(train[["dni_od_startu"]], train[CH4])

train["trend"] = model_trend.predict(train[["dni_od_startu"]])
test["trend"] = model_trend.predict(test[["dni_od_startu"]])   # ekstrapolacja trendu w przód

train["reszta"] = train[CH4] - train["trend"]
test["reszta"] = test[CH4] - test["trend"]   # tylko do ewaluacji, model tego nie widzi

print(f"Trend: CH4 = {model_trend.coef_[0]:.5f} * dni + {model_trend.intercept_:.2f}")
print(f"Wzrost roczny wg trendu: {model_trend.coef_[0] * 365:.2f} ppb/rok\n")
...
plt.plot(train["Data"], train["trend"], color="red", linewidth=2, label="Trend (dopasowany na treningu)")
plt.plot(test["Data"], test["trend"], color="red", linewidth=2, linestyle="--", label="Trend (ekstrapolacja)")
plt.xlabel("Data")
plt.ylabel("CH4 [ppb]")
plt.title("Trend liniowy CH4 w czasie")
plt.legend()
plt.tight_layout()
plt.savefig("ch4_trend_liniowy.png", dpi=150)
plt.show()
...
plt.tight_layout()
plt.savefig("ch4_detrend_lightgbm_rzeczywiste_vs_predykcja.png", dpi=150)
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig("ch4_detrend_szereg_czasowy_test.png", dpi=150)
plt.show()
...
for i, (idx_tr, idx_val) in enumerate(tscv.split(train)):
    ...
    # trend dopasowany tylko na fold_train
    mt = LinearRegression()
    mt.fit(fold_train[["dni_od_startu"]], fold_train[CH4])
    trend_val = mt.predict(fold_val[["dni_od_startu"]])
    reszta_fold_train = fold_train[CH4] - mt.predict(fold_train[["dni_od_startu"]])
...
plt.tight_layout()
plt.savefig("ch4_detrend_lightgbm_waznosc_cech.png", dpi=150)
plt.show()
...
plt.tight_layout()
plt.savefig("ch4_detrend_lightgbm_shap_summary.png", dpi=150, bbox_inches="tight")
plt.show()

print("\nGotowe. Zapisane pliki: ch4_trend_liniowy.png, "
      "ch4_detrend_lightgbm_rzeczywiste_vs_predykcja.png, "
      "ch4_detrend_szereg_czasowy_test.png, ch4_detrend_lightgbm_waznosc_cech.png, "
      "ch4_detrend_lightgbm_shap_summary.png")
